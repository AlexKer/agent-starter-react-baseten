...
class SpeechStream(stt.SpeechStream):
    ...
    *,
    ...
    async def _run(self) -> None:
        ...
        async def send_task(ws: aiohttp.ClientWebSocketResponse):
            ...
            async for data in self._input_ch:
                ...
                for frame in frames:
                    ...
                    # float_array = np.frombuffer(frame.data, dtype=np.float32)
                    # float_array = np.clip(float_array, -1.0, 1.0)
                    # int16_array = (float_array * 32767).astype(np.int16)

                    # logger.info(f"Sending {len(int16_array)} samples ({len(int16_array.tobytes())} bytes)")
                    # await ws.send_bytes(int16_array.tobytes())
        ...
        #                 # Optional: could emit an event or just log
        #                 logger.debug("Received silence event from Baseten")
